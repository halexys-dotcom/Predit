...
# Determinar quais os ficheiros a restaurar (mapeia origem local -> nome no device)
$pares = @{}
...
& $adb shell "mkdir -p $tempRemoto" 2>$null | Out-Null
foreach ($par in $pares.GetEnumerator()) {
    $origem = $par.Key
    $nomeDevice = $par.Value
    $remoto = "$tempRemoto/$nomeDevice"
    & $adb push $origem $remoto 2>$null | Out-Null
...
& $adb shell "run-as $Pacote rm -f databases/predit.db databases/predit.db-wal databases/predit.db-shm" 2>$null | Out-Null
...
& $adb shell "run-as $Pacote mkdir -p databases" 2>$null | Out-Null

# Copiar para databases/ usando run-as
foreach ($par in $pares.GetEnumerator()) {
    $nomeDevice = $par.Value
    $remoto = "$tempRemoto/$nomeDevice"
    $saida = & $adb shell "run-as $Pacote cp $remoto databases/$nomeDevice" 2>&1
    $cpExit = $LASTEXITCODE
    if ($cpExit -ne 0) {
...
& $adb shell "rm -rf $tempRemoto" 2>$null | Out-Null
...
$verificacao = & $adb shell "run-as $Pacote ls -l databases/predit.db" 2>&1
